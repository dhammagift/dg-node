...
// hyphenated English phrase like "non-self" is NOT an id (see looksLikeId — an id needs a digit or
// three or more parts), a Cyrillic question, a Chinese phrase, a Pali compound, a typo — all pass.
'use strict';
...
// bu-vb-sk7. Shape alone is not enough — "non-self" and "sutta-pitaka" have the same shape and are
// legitimate searches — so an id additionally needs a digit somewhere or at least three parts.
...
function looksLikeId(q) {
    ...
    if (!MULTIPART_ID.test(s)) return false;
    return /\d/.test(s) || s.split(/[-_]/).length >= 3;
}

// Returns a short reason string ('tipitaka_id', 'junk_chars', 'no_letters', 'consonant_soup',
// 'empty') when the string is not a search query, or null when it is one.

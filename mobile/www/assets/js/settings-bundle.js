// ---- public/overrides/js/settings.js ----
...
;
// ---- public/overrides/js/dg-text-router.js ----
...
(function (global) {
    ...
    function normalize(raw) {
        ...
        q = q.replace(/([a-z])\s+(\d)/g, '$1$2');
        // "sn 1 1" / "ыт 1  1" -> "sn1.1": пробел МЕЖДУ ЦИФРАМИ означает разделитель главы и
        // сутты, ровно как точка. Раньше склеивалась только пара "буква+цифра", а второе число
        // оставалось отдельным словом ("sn1 1"), не подходило ни под один шаблон classify() и
        // уезжало в обычный поиск по строке. То же делает normalizeTerm() в autopali.js для

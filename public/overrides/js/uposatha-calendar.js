...
(function () {
  ...
  function applyLang() {
    ...
    paintToc();
    $('help-new').href = t.helpUrl; if ($('help-frame').getAttribute('src')) $('help-frame').src = t.helpUrl + '?embed=1';
  ...
  var TOC = [['s-summary', 'tSummary'], ['slides', 'tSuttas'], ['s-calendar', 'tCalendar'], ['parts', 'tParts'], ['keys', 'tKeys'], ['help', 'tHelp'], ['soft', 'tSoft']];
  function paintToc() {
  ...
  function wire() {
    ...
    function toHelp(e) { if (e) e.preventDefault(); var c = document.querySelector('#dg-drawer .dg-drawer-close'); if (c && !$('dg-drawer').hidden) c.click(); $('help').scrollIntoView({ behavior: 'smooth', block: 'start' }); }
    $('b-help').onclick = $('d-help').onclick = toHelp;
    if ('IntersectionObserver' in window) new IntersectionObserver(function (es, ob) { if (es[0].isIntersecting) { ob.disconnect(); $('help-frame').src = t.helpUrl + '?embed=1'; } }, { rootMargin: '400px' }).observe($('help'));

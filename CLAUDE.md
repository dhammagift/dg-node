...
1. Положить в `siteroot/` symlink на реальные файлы (где бы они ни лежали — рядом с проектом,
   как `4nt`/`config`/`login`/`memo`/`read` сейчас: `siteroot/4nt -> ../../4nt`) ИЛИ прямо
   реальные файлы/папку, если контент небольшой и должен жить прямо в git-репозитории.
...
- Имя папки в `siteroot/` = URL-префикс один в один (`siteroot/dict` → `/dict/...`). Не
  занимайте имена, уже жёстко замаунченные явно ВЫШЕ по файлу до цикла сканирования
  `siteroot/`: `assets`, `spa`, `search` (алиас `/nodejs/res`), `nodejs`, `reader`. Если

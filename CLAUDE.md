...
из трёх мест, в этом порядке приоритета источника (DG-other побеждает SC при
совпадении ключа — раньше был race по `Promise.all`, теперь группы читаются
последовательно):

1. **SC** — `sc_bilara_data/translation/{lang}/*` (все переводчики SC).
2. **DG-main** — `assets/texts/{lang}/` — "лучший перевод проекта"; сейчас
...
3. **DG-other** — `assets/texts/{lang}_other/` — "второе мнение" проекта.

...
На проде старый (PHP, заморожен) репо лежит в `/var/www/html`, новый (dg-node) — в
`/var/www/dg-node-prod`, СНАРУЖИ DocumentRoot. Раньше он был подпапкой `/var/www/html/nodejs`, и
Apache по `Alias /old` отдавал `.env`, `configs/local/*`, `dg.db` и `.git` (issue #31); тест переехал
на `/var/www/dg-node-test` 2026-09-19, прод — 2026-09-25. Всё, что нужно dg-node из старого репо (ассеты,
`read/js/*` и т.п.), связано через **symlinks** — единый источник правды, без дублирования файлов
между репо; цели в `siteroot/` абсолютные (`/var/www/html/assets`), поэтому чекаут работает на
любой глубине.
...
- **Сначала тест, потом прод.** Правку проверяем в `dg-node-test` (test.dhamma.gift), и, если всё
  ок, выкатываем в прод сами: `git pull --ff-only` + `pm2 restart dg-prod` в
  `/var/www/dg-node-prod` (владелец разрешил, 2026-09-16). Рестарт нужен не только ради нового
  кода: бандлы (`home-bundle.js`, `settings-bundle.js`) пересобираются при старте сервера.
...
- **UI в приложении: сначала мокап, потом сборка.** Сборка APK/AAB идёт через CI и занимает
...
- **`configs/local/*.json` — обычные рабочие конфиги, но не для гита**: ключ Google TTS, Firebase
  web-конфиг, список допущенных к редактору переводов e-mail. В репозиторий они не попадают (ключ
  в публичном гите — утечка), на новой машине создаются руками; на боевой уже есть, `git pull` их
  не трогает (ignored).

...
(т.е. `nodejs/` — просто подпапка внутри старого репо, не рядом с ним). Тест с 2026-09-19 лежит
уже СНАРУЖИ — `/var/www/dg-node-test` (issue #31: внутри DocumentRoot Apache отдавал чекаут
файлами). Прод переедет туда же следующим шагом. Всё, что нужно dg-node из старого репо (ассеты,
...
- **Сначала тест, потом прод.** Правку проверяем в `dg-node-test` (test.dhamma.gift), и, если всё
  ...
  `/var/www/html/nodejs` (владелец разрешил, 2026-09-16). Рестарт нужен не только ради нового

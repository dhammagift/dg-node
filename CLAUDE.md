...
`public/assets` в этом репо, symlink на `../../assets` в проде). Если что-то 404-ит локально
под `/assets/...` — это не обязательно баг: проверь `C:\soft\dg\assets\...` (легаси-репо) и,
если файл там есть, скопируй его в `public/assets/...` (или `public/overrides/...`, если файл
...
│
├── read/                     — legacy TTS voice-player (symlink на легаси-репо, НЕ путать с reader/
│                                 выше — разные фичи с похожими именами, см. нюансы в TODO.md)
│
├── public/                   — статические файлы (CSS, JS, assets)
│   ├── overrides/js/         — файлы, которые мы реально правим (search-render.js, settings.js,
│   │                            dg-text-router.js, dhamma-i18n.js) — отдаются раньше легаси-assets
...
│
└── mirrors/                   — публикация тулз/зеркал от корня сайта, см. отдельный раздел ниже
    ...
    │                                 чужой) живёт ВНУТРИ mirrors/, configs/ — наши json-конфиги.
    └── (новые тулзы добавляются сюда же — см. раздел "Публикация тулз/зеркал" ниже)
```
...
## Публикация тулз/зеркал от корня сайта (`mirrors/`)

Отвечает на вопрос из TODO.md (общие п.3): как публиковать сторонние тулзы/зеркала сайтов
(4nt, dict, будущие) от корня (`dhamma.gift/{имя}/...`), не хардкодя каждую новую в
`dg-light.js` и не превращая корень репозитория в свалку, как в легаси.

**Как работает**: `dg-light.js` при старте сканирует `mirrors/` (`fsSync.readdirSync`) и для
КАЖДОЙ найденной записи — папки или symlink'а — автоматически регистрирует
`app.use('/{имя}', express.static(...))`. Значит, чтобы опубликовать новую тулзу:

1. Положить в `mirrors/` symlink на реальные файлы (где бы они ни лежали — рядом с проектом,
   как `4nt`/`config`/`login`/`memo` сейчас: `mirrors/4nt -> ../../4nt`) ИЛИ прямо реальные
   файлы/папку, если тулза небольшая и должна жить прямо в git-репозитории.
...
- Список папок сканируется ОДИН РАЗ при старте процесса, не на каждый запрос — новое имя в
  `mirrors/` требует рестарта сервера. Правки ВНУТРИ уже примонтированной тулзы (файлы
  изменились, но имя папки то же) видны сразу, рестарт не нужен — `express.static` читает с
  диска на лету.
- Имя папки в `mirrors/` = URL-префикс один в один (`mirrors/dict` → `/dict/...`). Не занимайте
  имена, уже жёстко замаунченные явно ВЫШЕ по файлу до цикла сканирования `mirrors/`: `assets`,
  `spa`, `search` (алиас `/nodejs/res`), `nodejs`, `reader`, `read`. Если совпадёт — явный
  маунт (он регистрируется раньше) победит, запись в `mirrors/` будет молча проигнорирована.
- `/ru/login`, `/ru/memo` — единственное исключение из правила "никакого хардкода": это не
  ...
  прописаны явно двумя строчками сразу после цикла сканирования `mirrors/`.
- Технический нюанс, который стоит знать при отладке (если новая тулза вдруг "не находится"):
...
работает по тому же принципу (скан папки → авто-маунт), но `mirrors/` — специально ВНУТРИ
